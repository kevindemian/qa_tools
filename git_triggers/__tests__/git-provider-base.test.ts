...
class TestProvider extends GitProviderBase {
    ...
    async publicGet(url: string, opts?: { operation?: string; params?: Record<string, unknown> }) {
...
describe('GitProviderBase._get', () => {
    ...
    it('throws ExternalError on error', async () => {
        ...
        await expect(provider.publicGet('/test')).rejects.toBeInstanceOf(ExternalError);
    });
});

...
export function aggregatePipelineHealth(
    ...
    /* Failure reasons — deduplicate across all error messages */
    ...
    const openIssues = { total: issues.length, byLabel, staleCount };
...
export function renderPipelineHealthHtml(health: PipelineHealth, title = 'Pipeline Health Report'): string {
    ...
    const bar = (rate: number, maxWidth = 12): string => {
        const filled = Math.round((rate / 100) * maxWidth);
        return '█'.repeat(filled) + '░'.repeat(maxWidth - filled);
    };

    let jobsHtml = '<p>No failing jobs</p>';
    if (health.topFailingJobs.length > 0) {
        jobsHtml =
            '<table border="1" cellpadding="6" style="border-collapse:collapse;width:100%">' +
            '<tr style="background:#f3f4f6"><th>Job</th><th>Failed</th><th>Total</th><th>Rate</th><th></th></tr>' +
            health.topFailingJobs
                .map(
                    (j) =>
                        `<tr>` +
                        ...
                        `<td>${j.failCount}</td>` +
                        `<td>${j.totalCount}</td>` +
                        `<td><span style="color:${statusColor('failure')}">${j.rate}%</span></td>` +
                        `<td style="font-family:monospace;font-size:0.8rem;color:#6b7280">${bar(j.rate)}</td>` +
                        `</tr>`,
                )
                .join('\n') +
            '</table>';
    ...
    let reasonsHtml = '<p>No failure reasons captured</p>';
    if (health.failureReasons.length > 0) {
        reasonsHtml =
            '<table border="1" cellpadding="6" style="border-collapse:collapse;width:100%">' +
            '<tr style="background:#f3f4f6"><th>Count</th><th>Error Message</th></tr>' +
            health.failureReasons
                .map(
                    (r) =>
                        `<tr>` +
                        `<td>${r.count}x</td>` +
                        `<td style="font-family:monospace;font-size:0.8rem">${escapeHtml(r.message)}</td>` +
                        `</tr>`,
                )
                .join('\n') +
            '</table>';
    ...
    let catHtml = '<p>No categorized failures</p>';
    if (catKeys.length > 0) {
        ...
        catHtml = '<div style="display:flex;flex-wrap:wrap;gap:0.75rem">';
        for (const key of catKeys) {
            catHtml +=
                ...
                `<div style="font-size:1.5rem;font-weight:700;color:${catColors[key] ?? '#1f2937'}">${health.failureByCategory[key]}</div>` +
                `</div>`;
        }
        catHtml += '</div>';
    }
    ...
    let branchHtml = '';
    if (health.branchBreakdown.length > 0) {
        branchHtml =
            '<table border="1" cellpadding="6" style="border-collapse:collapse;width:100%;margin-top:0.5rem">' +
            '<tr style="background:#f3f4f6"><th>Branch</th><th>Runs</th><th>Pass Rate</th></tr>' +
            health.branchBreakdown
                .map(
                    (b) =>
                        `<tr>` +
                        `<td><strong>${escapeHtml(b.branch)}</strong></td>` +
                        `<td>${b.count}</td>` +
                        `<td><span style="color:${statusColor(b.passRate >= 80 ? 'success' : 'failure')}">${b.passRate}%</span></td>` +
                        `</tr>`,
                )
                .join('\n') +
            '</table>';
    ...
    let issuesHtml = `<p><strong>${issues.total}</strong> open, <strong>${issues.staleCount}</strong> stale (30d+ sem atividade)</p>`;
    if (Object.keys(issues.byLabel).length > 0) {
        issuesHtml +=
            '<table border="1" cellpadding="6" style="border-collapse:collapse;width:auto">' +
            '<tr style="background:#f3f4f6"><th>Label</th><th>Count</th></tr>' +
            Object.entries(issues.byLabel)
                .sort((a, b) => b[1] - a[1])
...
<div class="summary">
  ...
  <div class="card"><div class="num" style="color:${health.passRate >= 80 ? '#16a34a' : '#dc2626'}">${health.passRate}%</div><div class="lbl">Pass Rate</div></div>

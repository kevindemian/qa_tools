par# Data Hub — Arquitetura em Camadas (v2)
...
## Fase 14 — Correção de Bugs de Integração + Gaps de Teste (Tarefas 170-186)

> **Início:** 2026-07-05
>
> **Motivação:** Três bugs de integração foram identificados após Fase 13:
>
> 1. Quality Gate com coverage=0 (dados incorretos)
> 2. PR Report com tabelas mostrando valores incorretos
> 3. DataHub nunca é criado no CI (async/sync mismatch)
>
> **Causa raiz:** `createGitProvider()` é async mas `tryCreateDataHub()` chama `providerFactory(ciEnv)` sem `await`. Cast `as unknown` em `pr-report-entry.ts` esconde o bug. Resultado: DataHub nunca criado, fallback para MetricsStore vazio.
...
| Questão                                          | Solução                                                                             |
| ------------------------------------------------ | ----------------------------------------------------------------------------------- |
| `createGitProvider` é async por `await import()` | Tornar síncrono com imports estáticos (padrão da Fase 13 em `session-state.ts:227`) |
| Cast `as unknown` em `pr-report-entry.ts`        | Remover após factory síncrono — tipo agora confere                                  |
| `_resolveCoverage` não usa DataHub               | Adicionar `dataHub.computed.coverage` como último fallback                          |
| Coverage artifact não existe no CI               | Upload/download entre jobs via `actions/upload-artifact@v4`                         |
| ZERO testes para factory/wiring                  | Criar suite completa seguindo RED→GREEN→REFACTOR                                    |

#### Fluxo de Correção

```
┌─────────────────────────────────────────────────────────────────────┐
│ PHASE 5 (RED) — Criar testes que FALHAM contra código atual        │
│                                                                     │
│ Para cada gap: teste que reproduz o bug/gap                         │
│ Expected values de REQUISITOS, nunca de output atual                │
│ Mock shape IDÊNTICO ao real                                         │
│ PBT obrigatório para lógica crítica                                 │
├─────────────────────────────────────────────────────────────────────┤
│ PHASE 6 (GREEN) — Corrigir código para passar nos testes           │
│                                                                     │
│ Ordem: T14 (suppressions) → tsc → tipos → demais                   │
│ Proibido: alterar expected values, workaround, bypass               │
├─────────────────────────────────────────────────────────────────────┤
│ PHASE 7 — Integração                                               │
│                                                                     │
│ Todos os consumers intactos, full suite sem regressões              │
└─────────────────────────────────────────────────────────────────────┘
```

---

### 170 — Tornar `createGitProvider` SÍNCRONO

| Item               | Conteúdo                                                                                                    |
| ------------------ | ----------------------------------------------------------------------------------------------------------- |
| **Gap**            | `createGitProvider()` retorna `Promise<GitProvider>` mas `tryCreateDataHub()` espera `GitProvider` síncrono |
| **Arquivo**        | `git_triggers/git-provider-factory.ts`                                                                      |
| **Mudança**        | Imports dinâmicos `await import()` → imports estáticos no topo. `async function` → `function`               |
| **Trade-off**      | `git_triggers/` pode importar de `git_triggers/` (mesma camada). Sem layer violation                        |
| **Teste unitário** | `git_triggers/__tests__/git-provider-factory.test.ts` — 6 cenários                                          |
| **PBT**            | `git_triggers/__tests__/git-provider-factory.property.test.ts` — output é `GitProvider                      | undefined` |
| **Critério**       | `npx vitest run git_triggers/__tests__/git-provider-factory` = 100% pass                                    |

**Tarefa 170a — RED:** Criar testes que FALHAM com factory async

| #   | Teste                                    | Esperado                         | Status Atual             |
| --- | ---------------------------------------- | -------------------------------- | ------------------------ |
| R1  | `ciEnv.isCI = false`                     | Retorna `undefined`              | ⚠️ Precisa mock env      |
| R2  | Nenhum token configurado                 | Retorna `undefined`              | ⚠️ Precisa limpar env    |
| R3  | `CI_JOB_TOKEN` + `CI_PROJECT_ID` setados | Retorna `GitLabManager`          | ❌ Async retorna Promise |
| R4  | `GITHUB_TOKEN` setado (sem GitLab)       | Retorna `GitHubManager`          | ❌ Async retorna Promise |
| R5  | `CI_SERVER_URL` ausente                  | Usa default `https://gitlab.com` | ❌ Async                 |
| R6  | Ambos tokens setados (GitLab priority)   | Retorna `GitLabManager`          | ❌ Async                 |

**Tarefa 170b — GREEN:** Corrigir factory para síncrono
...
### 171 — Remover Cast `as unknown` em `pr-report-entry.ts`

| Item               | Conteúdo                                                                               |
| ------------------ | -------------------------------------------------------------------------------------- |
| **Gap**            | Cast `(main as (providerFactory?: unknown) => Promise<void>)` esconde mismatch de tipo |
| **Arquivo**        | `git_triggers/pr-report-entry.ts`                                                      |
| **Mudança**        | `return main(createGitProvider)` direto, sem cast                                      |
| **Teste unitário** | `git_triggers/__tests__/pr-report-entry.test.ts` — 3 cenários                          |
| **Critério**       | `npx vitest run git_triggers/__tests__/pr-report-entry` = 100% pass                    |

**Tarefa 171a — RED:** Criar testes que falham com cast

| #   | Teste                                         | Esperado                  |
| --- | --------------------------------------------- | ------------------------- |
| R1  | `mainWithProvider()` chama `main` com factory | Verificar argumento       |
| R2  | Factory recebida é `createGitProvider`        | Comparar referência       |
| R3  | Erro propagado                                | `process.exit(1)` chamado |

**Tarefa 171b — GREEN:** Remover cast
...
### 172 — Testes de Wiring: `tryCreateDataHub` + `main(providerFactory)`

| Item         | Conteúdo                                                                  |
| ------------ | ------------------------------------------------------------------------- |
| **Gap**      | 25 linhas de branching logic sem teste (shared/pr-report-core.ts:667-692) |
| **Arquivo**  | `shared/__tests__/pr-report-core.wiring.test.ts`                          |
| **Testes**   | 5 cenários + PBT                                                          |
| **Critério** | `npx vitest run shared/__tests__/pr-report-core.wiring` = 100% pass       |

**Tarefa 172a — RED:** Testes que reproduzem o bug de wiring

| #   | Teste                                             | Esperado                                     |
| --- | ------------------------------------------------- | -------------------------------------------- |
| R1  | `main(factory)` factory retorna `undefined`       | DataHub não criado, fallback MetricsStore    |
| R2  | `main(factory)` factory retorna `GitProvider`     | DataHub criado, passed to `generatePrReport` |
| R3  | `main(factory)` factory lança exceção             | Erro tratado, fallback MetricsStore          |
| R4  | `main(factory)` `getOrFetchDataHub` lança exceção | Erro tratado, fallback MetricsStore          |
| R5  | `ciEnv.isCI = false`                              | `tryCreateDataHub` retorna `undefined`       |

**Tarefa 172b — GREEN:** Não requer mudança adicional (após Gap 170)

**Tarefa 172c — PBT:** `shared/__tests__/pr-report-core.wiring.property.test.ts`

- Propriedade: factory sempre recebida como `GitProvider` (nunca `Promise`)
- Propriedade: output de `main` é sempre `void` (nunca propaga exceção)

---

### 173 — Testes: Jira Provider Field Mapping

| Item         | Conteúdo                                                                       |
| ------------ | ------------------------------------------------------------------------------ |
| **Gap**      | 3 campos mapeados mas não testados (`created`, `updated`, `resolution`)        |
| **Arquivo**  | `shared/data-hub/__tests__/providers/jira-provider.test.ts` (adicionar)        |
| **Critério** | `npx vitest run shared/data-hub/__tests__/providers/jira-provider` = 100% pass |

**Tarefa 173a — RED:**

| #   | Teste                                   | Esperado                |
| --- | --------------------------------------- | ----------------------- |
| R1  | `fields.created` → `created`            | ISO string              |
| R2  | `fields.updated` → `updated`            | ISO string              |
| R3  | `fields.resolution.name` → `resolution` | String                  |
| R4  | `fields.resolution` ausente             | `resolution: undefined` |

**Tarefa 173b — GREEN:** Não requer mudança (mapeamento já implementado)

---

### 174 — Testes: Composite Provider Dynamic Source + Merge

| Item         | Conteúdo                                                                            |
| ------------ | ----------------------------------------------------------------------------------- |
| **Gap**      | Dynamic source e first-writer-wins não testados                                     |
| **Arquivo**  | `shared/data-hub/__tests__/providers/composite-provider.test.ts` (adicionar)        |
| **Critério** | `npx vitest run shared/data-hub/__tests__/providers/composite-provider` = 100% pass |

**Tarefa 174a — RED:**

| #   | Teste                                  | Esperado                                   |
| --- | -------------------------------------- | ------------------------------------------ |
| R1  | `source` reflete primeiro provider     | `composite.source === providers[0].source` |
| R2  | Coverage merge: primeiro writer wins   | Primeiro provider prevalece                |
| R3  | jiraIssues merge: primeiro writer wins | Primeiro provider prevalece                |
| R4  | Ordem de providers importa             | Trocar ordem → resultado diferente         |

**Tarefa 174b — GREEN:** Não requer mudança

---

### 175 — DataHub Coverage Fallback no Quality Gate

| Item         | Conteúdo                                                                         |
| ------------ | -------------------------------------------------------------------------------- |
| **Gap**      | `_resolveCoverage()` não usa `dataHub.computed.coverage` → coverage=0            |
| **Arquivo**  | `shared/health-score.ts`                                                         |
| **Mudança**  | Adicionar `dataHub?.computed?.coverage` como último fallback antes de retornar 0 |
| **Teste**    | `shared/__tests__/health-score.test.ts` — 4 cenários de prioridade               |
| **Critério** | `npx vitest run shared/__tests__/health-score` = 100% pass                       |

**Tarefa 175a — RED:** Teste que reproduz coverage=0

| #   | Teste                                           | Esperado                    |
| --- | ----------------------------------------------- | --------------------------- |
| R1  | Coverage override definido                      | Usa override (prioridade 1) |
| R2  | Sem override, history existe                    | Usa history (prioridade 2)  |
| R3  | Sem override, sem history, DataHub com coverage | Usa DataHub (prioridade 3)  |
| R4  | Sem tudo                                        | Retorna 0 (prioridade 4)    |

**Tarefa 175b — GREEN:** Corrigir `_resolveCoverage`

```typescript
function _resolveCoverage(config, store, dataHub?): number {
    if (config.coverageOverride !== undefined) return config.coverageOverride;
    if (store.coverageHistory?.length > 0) {
...
### 176 — Coverage Artifact no CI (via Generators)

| Item              | Conteúdo                                                                                               |
| ----------------- | ------------------------------------------------------------------------------------------------------ |
| **Gap**           | `coverage/coverage-summary.json` não existe no post-process                                            |
| **Abordagem**     | Modificar os **generators** que produzem os YAMLs, não os YAMLs manualmente                            |
| **Generators**    | `setup/templates/github-ci.ts`, `setup/templates/qa-post-process-workflow.ts`, `shared/ci-injector.ts` |
| **YAMLs gerados** | `.github/workflows/ci.yml`, `.github/workflows/qa-post-process.yml`                                    |
| **Critério**      | Contract tests passam (deployed = generator) + artifacts disponíveis                                   |

**Tarefa 176a — Reverter YAMLs manuais:**

- `git checkout .github/workflows/ci.yml`
- `git checkout .github/workflows/qa-post-process.yml`

**Tarefa 176b — Modificar `setup/templates/github-ci.ts`:**
Adicionar step de upload de coverage **após** o upload de CTRF (dentro do `if (ctx.features.prReport)`):
...
if (ctx.features.prReport) {
    testSteps.push({
        ...
        with: { name: 'ctrf-report', path: ctx.ctrfReportPath, 'if-no-files-found': 'warn' },
    });
    // NOVO: upload de coverage
    testSteps.push({
        name: 'Upload coverage report',
        uses: 'actions/upload-artifact@v4',
        with: { name: 'coverage-report', path: 'coverage/', 'if-no-files-found': 'warn' },
    });
}
```

**Tarefa 176c — Modificar `setup/templates/qa-post-process-workflow.ts`:**
Adicionar step de download de coverage **após** o download de CTRF:
...
**Tarefa 176d — Modificar `shared/ci-injector.ts`:**
Adicionar o mesmo step em `generatePostProcessWorkflowYaml()`:
...
**Tarefa 176e — Atualizar testes:**

| Arquivo                                            | Novo teste                                                     |
| -------------------------------------------------- | -------------------------------------------------------------- |
| `setup/templates/github-ci.test.ts`                | Verificar `coverage-report` no YAML quando prReport habilitado |
| `setup/templates/qa-post-process-workflow.test.ts` | Verificar download de `coverage-report`                        |
| `shared/ci-injector.test.ts`                       | Verificar download de `coverage-report` + contract test        |

**Tarefa 176f — Regenerar YAMLs deployed:**
Chamar generators para produzir os YAMLs atualizados e escrever nos arquivos deployed.

---

### 177 — Corrigir Path Errado em `generateQaPostProcessAction`

| Item             | Conteúdo                                                                        |
| ---------------- | ------------------------------------------------------------------------------- |
| **Gap**          | `github-ci.ts:34` gera `run: npx tsx shared/pr-report-core.ts` — path incorreto |
| **Path correto** | `git_triggers/pr-report-entry.ts`                                               |
| **Arquivo**      | `setup/templates/github-ci.ts` (função `generateQaPostProcessActionYaml`)       |
| **Teste**        | `setup/templates/github-ci.test.ts` — teste asserts path correto                |
...
### 178 — Atualizar Action Versions em `github-ci.ts`

| Item                 | Conteúdo                                                            |
| -------------------- | ------------------------------------------------------------------- |
| **Gap**              | `github-ci.ts` usa `@v4` para checkout, setup-node, upload-artifact |
| **Versões corretas** | `checkout@v5`, `setup-node@v6`, `upload-artifact@v7`                |
| **Arquivo**          | `setup/templates/github-ci.ts`                                      |
...
// Linha 54-55: setup-node
{ uses: 'actions/setup-node@v6', with: { 'node-version': ctx.nodeVersion } };
// Linha 65: upload-artifact CTRF
uses: 'actions/upload-artifact@v7'
// Linha 72: upload-artifact coverage
uses: 'actions/upload-artifact@v7'
```

---

### 179 — Suporte a Schedule Exclusion no Injector

| Item        | Conteúdo                                                                                        |
| ----------- | ----------------------------------------------------------------------------------------------- |
| **Gap**     | Injector gera `if: always()` mas deployed tem `if: always() && github.event_name != 'schedule'` |
| **Arquivo** | `shared/ci-injector.ts`                                                                         |
| **Solução** | Adicionar parâmetro `excludeSchedule?: boolean` (default `true`)                                |

**Mudança em `injectPostProcessJob`:**
...
**Mudança em `generatePostProcessWorkflowYaml`:**

```typescript
export function generatePostProcessWorkflowYaml(options: PostProcessWorkflowOptions): string {
    // ...
    // Linha 70: adicionar condition no step "Run QA Tools Post-Processing"
    '      - name: Run QA Tools Post-Processing',
    '        if: always()',
    // (já está correto — o condition é no job, não no step)
}
```
...
```
┌─────────────────────────────────────────────────────────────────────┐
│ 170 (factory sync) → 171 (remover cast) → 172 (wiring tests)      │
│                                                                    │
│ 173 (jira fields) ──────────────────────────┐                     │
│ 174 (composite merge) ──────────────────────┤ paralelos           │
│ 175 (coverage fallback) ────────────────────┤ (sem dependência)   │
│ 176 (CI artifact generators) ───────────────┤                     │
│ 177 (composite action path) ────────────────┤                     │
│ 178 (action versions) ─────────────────────┤                     │
│ 179 (schedule exclusion) ──────────────────┘                     │
├─────────────────────────────────────────────────────────────────────┤
│ Quality Gates: tsc → lint → vitest → PBT                          │
├─────────────────────────────────────────────────────────────────────┤
│ Regenerar YAMLs deployed a partir dos generators                   │
├─────────────────────────────────────────────────────────────────────┤
│ Commit único: fix(data-hub): resolve integration + test gaps       │
└─────────────────────────────────────────────────────────────────────┘
```
...
| ------ | ------------ | -------------- | ------------ | ----------- |
| 170    | ✅ 0 erros   | ✅ 0 violações | ✅ 100% pass | ✅ presente |
| 171    | ✅ 0 erros   | ✅ 0 violações | ✅ 100% pass | —           |
| 172    | ✅ 0 erros   | ✅ 0 violações | ✅ 100% pass | ✅ presente |
| 173    | ✅ 0 erros   | ✅ 0 violações | ✅ 100% pass | —           |
| 174    | ✅ 0 erros   | ✅ 0 violações | ✅ 100% pass | —           |
| 175    | ✅ 0 erros   | ✅ 0 violações | ✅ 100% pass | —           |
| 176    | —            | —              | —            | —           |
| 177    | ✅ 0 erros   | ✅ 0 violações | ✅ 100% pass | —           |
| 178    | ✅ 0 erros   | ✅ 0 violações | ✅ 100% pass | —           |
| 179    | ✅ 0 erros   | ✅ 0 violações | ✅ 100% pass | —           |

**Checkpoint Fase 14:** `npx vitest run` = 100% pass. Factory síncrono. DataHub criado. Coverage real. Quality Gate correto. PR Report com dados corretos. Templates corretos. YAMLs deployed = generators.
**Commit:** `fix(data-hub): resolve async/sync mismatch, fix CI templates, add test gaps`

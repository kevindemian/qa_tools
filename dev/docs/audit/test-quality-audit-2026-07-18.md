...
|-----------|------------------|---------------------|---------------------|
| `git_triggers` | **42** | ⏳ **EM ABERTO** — auditoria manual dois-a-dois (source+teste) em curso. Ver `audit/test-audit/PROGRESS.md`. Marcações "CONCLUÍDO" anteriores por triagem de assinatura foram INVALIDADAS por §20.6 (2026-07-19). | — |
| `jira_management/commands/__tests__` | 33 | ⏳ **EM ABERTO** — auditoria manual dois-a-dois em curso (§20.6). | — |
| `jira_management/__tests__` | 40 | ⏳ **EM ABERTO** — auditoria manual dois-a-dois em curso (§20.6). | — |
| `shared` | **356** (`__tests__/` + `data-hub/__tests__/` + `invariants/__tests__/` + `primitives/__tests__/` + `test-utils/__tests__/`) | ⏳ **EM ANDAMENTO** — 118 arquivos já auditados manualmente dois-a-dois (B1.1 + B1.2a + parte B1.2b, ver PROGRESS.md); 245 restantes de `shared/` + TODOS os demais domínios aguardando leitura integral. Marcação "CONCLUÍDO por triagem" INVALIDADA por §20.6. | 6 (T3, corrigidos na origem, em B1.2a/B1.2b) |
| `scripts/__tests__` | 8 | ⏳ **EM ABERTO** — auditoria manual dois-a-dois em curso (§20.6). | — |
| `setup/__tests__` | 14 | ⏳ **EM ABERTO** — auditoria manual dois-a-dois em curso (§20.6). | — |
| `e2e` | 13 | ⏳ **EM ABERTO** — auditoria manual dois-a-dois em curso (§20.6). | — |
| **TOTAL** | **506 test files** (escopo real, `*.test.ts`/`*.spec.ts`) | ⏳ **AUDITORIA EM ANDAMENTO** — retomada manual arquivo-a-arquivo conforme §20.6 / §21. NENHUMA conclusão por amostragem/triagem é válida. Fonte da verdade do ponto exato: `audit/test-audit/PROGRESS.md`. | 6 (T3, em `shared/`) |
...
> ⛔ **INVALIDAÇÃO (§20.6 / §21, 2026-07-19):** Todo o bloco de resultados por domínio abaixo
> (linhas seguintes até §20.6) foi produzido por **triagem de assinatura / leitura parcial de
> clusters de risco**, NÃO por leitura manual integral arquivo-a-arquivo em pares source+teste.
> Isso viola a regra §16/§20.1 e foi expressamente descartado pelo usuário. **Nenhuma das
> marcações "CONCLUÍDO / TODOS SÃO / AUDITADO SÃO" abaixo é válida.** A auditoria foi
> reiniciada do zero: leitura manual dois-a-dois de TODOS os 506 arquivos de teste. Fonte da
> verdade do ponto exato e do estado: **`audit/test-audit/PROGRESS.md`**. O conteúdo abaixo é
> mantido SOMENTE como histórico de tentativa inválida, não como evidência de conformidade.
...
> ⛔ **STATUS INVALIDADO (§20.6 / §21):** a marcação "✅ AUDITORIA CONCLUÍDA" abaixo é
> **FALSA** — baseou-se em triagem de assinatura / leitura parcial, proibida por §16/§20.1.
> O usuário determinou (2026-07-19) que TODOS os 506 arquivos estão EM ABERTO e a auditoria
> deve ser manual, arquivo-a-arquivo, em pares source+teste. O STATUS real é **EM ANDAMENTO**.

**STATUS (2026-07-20, CORRIGIDO): ⏳ AUDITORIA EM ANDAMENTO — RETOMADA MANUAL DOIS-A-DOIS.**
A auditoria manual arquivo-a-arquivo está em curso conforme §20.6/§21. Até o momento:
118 arquivos de `shared/` auditados manualmente (B1.1 + B1.2a + parte B1.2b, ver
`audit/test-audit/PROGRESS.md`) com 6 issues T3 corrigidos na origem; 245 arquivos restantes
de `shared/` + TODOS os 150 arquivos de `git_triggers`/`jira_management`/`scripts`/`setup`/`e2e`
ainda EM ABERTO (aguardando leitura integral). Os 7 defeitos de produção citados
(`0222380c`, `2f517247`, `fed20a21`, `878755a3`, `633bbeb7`, `f5dc3be8`, `313fcdd9`) e o
teatro `fbdaa854` FORAM corrigidos na origem e permanecem corrigidos — mas NÃO constituem
auditoria concluída dos testes. Ressalvas documentadas (XSS `shared/primitives/*`,
`formatDuration(NaN)`) seguem pendentes de decisão de contrato. Fonte da verdade: PROGRESS.md.
...
("AUDITAR = todos os arquivos de teste do repo; auditoria varre todos"). **Esta conclusão
está INVALIDADA** — ver aviso de invalidação acima e `audit/test-audit/PROGRESS.md`.

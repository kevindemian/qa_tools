...
**Created:** 2026-07-31
**Status:** Auditoria completa — 24 artefatos × 3 dimensões (D1 ✅ 24 · D2 ✅ 24 · D3 ✅ 24) · Fase I ✅ · Fase II (protocolo) ✅ · Fase III (validação determinística) ✅ · Fase IV (encerramento) ✅ — ENCERRADO 2026-08-05
...
| D5 | Determinismo como regra (Q5) | **`resolveGeneratedAt(seed)` canônico** (I-9.6). Teste de arquitetura: scan renderers → `new Date()` cru no output falha. `VALIDATION-PLAN.md` exige o invariante. ✅ (2026-08-05): `report-determinism.architecture.test.ts` + migrations; sha256 idêntico 2×; D3 passo do protocolo. |
...
| 19 | pipeline-health | ✅ | ✅ | ✅ | Consumido por `batch-mode.ts:445`. **I-8/F6/B14 (2026-08-04):** CSS custom `_PIPELINE_CSS` + 6 `style=` inline + 3 emojis migrados → primitives/tokens (`shared/primitives/report-styles.ts` `PIPELINE_HEALTH_CSS`); cores via `data-color`/`data-status` attrs; emojis→`icon()` SVG com `aria-label`; a11y (skip-link/focus) preservada. Zero inline/emoji/hex fora de tokens. Harness regenerado (`reports/validation/pipeline-health.html`). |
| 20 | schedule-handler | ✅ | ✅ | ✅ | **FIXED I-1 (2026-08-03, commit `63c0f19a`):** consome `hub.computed.*` (`defectAggregation`, `traceabilityTree`, `backlogHealth`, `developerProfile`, `aiComparison`, `pipelineCostResult`); recomputação local e fallback `generateGitMetricsRuns` removidos; guards invariantes (Rule 24/25); `<2 runs` warn+return. Gate completo verde (tsc/lint/vitest 539×7474/ts-prune/depcruise). Débitos D-1/D-2 absorvidos na execução de I-2 (§11.2). Zero débito remanescente. |
| 21 | interactive-mode | ✅ | ✅ | ✅ | Consome SSOT para `aiMetrics`/`passRate`/`flakyRate`/`coverage` ✅. **I-2 (2026-08-03, `02b2c652`):** 100% `hub.computed.*` (defectAggregation/traceabilityTree/seasonality/regression/optimization/backlogHealth/developerProfile/aiComparison/pipelineCostResult/coverageGap) — recomputação local + fallback git removidos. **I-8.2 (2026-08-05, `26ef64d6`):** `_dashboardQualityGate` → `buildHtmlPage`/`buildCss`/`resolveGeneratedAt` (padrão dos 16 irmãos); zero HTML cru. |
...
| 23 | pr-report-job-summary | ✅ | ✅ | ✅ | Escrito em `$GITHUB_STEP_SUMMARY`. **B18 (2026-08-03, F5-T5):** emojis (`:white_check_mark:`, `:x:`, `:fast_forward:`, `:clock1:`, `:warning:`, `:large_blue_diamond:`) → `MARKDOWN_SYMBOLS` ASCII (`[PASS]/[FAIL]/[WARN]/...`) — zero codepoints emoji, visual-checklist item "zero emojis" conforme. |
...
### P3 — SSOT violation sistêmica (D3) — **FIXED I-1 + I-2 (2026-08-03)**
...
- **Correção de origem (aplicada — I-1 + I-2):** schedule-handler (`63c0f19a`) e interactive-mode (`02b2c652` + `1a6e6ab2`) consomem `dataHub.computed.*`; recomputação local e fallback `generateGitMetricsRuns`/`getLastGitLogError` removidos; dedup D-2 consolidado em `compute/defect-aggregation.ts` (quality vira barrel renderer).

### P4 — Forma (D3)
- ~~`pipeline-health-renderer.ts`: 6 inline styles + `_PIPELINE_CSS` custom (único renderer).~~ → **I-8/F6/B14 (2026-08-04):** integrado em `PIPELINE_HEALTH_CSS` (primitives/report-styles.ts), zero inline styles. Nenhum renderer emite `style=` inline estático.
- ~~`interactive-mode.ts:579` quality-gate: HTML cru inline.~~ → **I-8.2 (2026-08-05, `26ef64d6`):** `_dashboardQualityGate` → `buildHtmlPage` + primitives. Zero HTML cru.
- ~~`pr-report-job-summary`: emojis.~~ → **B18/F5-T5 (2026-08-03):** `MARKDOWN_SYMBOLS` ASCII; zero emoji no job-summary.
...
- **CONTENT-SPECIFICATION.md gap table (linhas 557-576, 2026-07-11) — DESATUALIZADA, CORRIGIDA (2026-08-05, Fase IV):** afirmava timestamp só em traceability e thresholds em nenhum — todos os 16 dashboards agora têm timestamp ✅ + thresholds ✅ (verificado no HTML gerado). Tabela atualizada com nota + linhas riscadas.
- **Caminhos fantasma em documentos — CORRIGIDOS (2026-08-05, Fase IV):** TECHDOC:204 já não referencia `coverage-gap-renderer.ts`; INTEGRATED-PLAN FT-11 `shared/coverage-source.ts` → `shared/data-hub/compute/coverage.ts`; FT-101 `shared/artifact-validator.ts` → `shared/validation/artifact-validator.ts`. Varredura completa do INTEGRATED-PLAN (Grupos 0-3): 30+ refs stale a módulos movidos (`shared/quality/`, `shared/report/`, `shared/validation/`, `shared/infra/`, `shared/data-hub/compute/`) atualizadas; FT-37 (git-metrics-adapter) marcado removido (I-2 D2-delete). Zero GHOST.
- **Completion-plan "28 artefatos" desatualizado — CORRIGIDO (2026-08-05, Fase IV):** SSOT `artifact-specs.ts` define 24; todas as ocorrências "28/28"/"28 artefatos" atualizadas para 24 (nota R2 "21 + 3 pr-report" já era 24).
...
1. ✅ Nenhum primitive emite `style=` inline (exceto larguras dinâmicas permitidas pelo checklist item 10). **I-8/F6/B14 (2026-08-04):** pipeline-health migrado → zero renderers com inline styles estáticos (apenas larguras/cores dinâmicas de dados).
...
| Forma residual | ~~`pipeline-health-renderer.ts` (50-58, 6 style=, 3 emojis)~~ → **I-8/F6/B14 (2026-08-04) resolvido** | CSS custom + inline + emoji |
...
| **F5** | F5-T1..T6 | Tokens AA (light→**Primer**, decisão §10: `success→#1a7f37`, `error→#d1242f`, `warn→#9a6700`, `info→#0969da`); focus-visible/reduced-motion/tabular-nums/color-mix/skip-link; classificar 15 `style=`; emojis→texto/ASCII; golden reference (OmniRoute) | `rg` zero hex fora de tokens; zero emoji | ✅ implementado (I-0.5) + validado (Fase III, 2026-08-05) |
...
| Pendentes verificado no código | F0-T6, F0-T7, F0-T11 ✅, F0-T12 ✅, F2, F3 ✅, F4, F6-T2/T3 ✅, F7 | `schedule-handler.ts:182/183`, `interactive-mode.ts:570`, `pr-report-core.ts:89`, `report-sections.ts` 8x `return ''`, AQS ausente. ~~`pipeline-health-renderer.ts:49/75/97/140-143`~~ → **I-8 ✅ (2026-08-04)**. ~~`schedule-handler.ts`/`interactive-mode.ts` raw HTML~~ → **I-8.2 ✅ (2026-08-05, `26ef64d6`)**. ~~Validação determinística~~ → **Fase III ✅ (2026-08-05)** |
| Validação determinística | ✅ executada/relatada (Fase III, 2026-08-05) | `dev/docs/internal/VALIDATION-REPORT.md`: 21 hashes sha256 de `reports/validation/*.html` idênticos entre 3 runs; suíte 543 files/7481 tests; EXIT=0; determinismo PROVADO |
...
**Tarefas executável:**
- ~~`I-8.1` `git_triggers/pipeline-health-renderer.ts`: remover `_PIPELINE_CSS` (`:49`) + inline styles (`:75,97,140-143`) -> primitivas (`shared/primitives`/`report-styles`/tokens); a11y preservada.~~ → **✅ DONE (2026-08-04):** `_PIPELINE_CSS` + 6 inline styles + 3 emojis migrados para `PIPELINE_HEALTH_CSS` (`shared/primitives/report-styles.ts`); cores via `data-color`/`data-status`; emojis→`icon()` SVG com `aria-label`; a11y (skip-link/focus) preservada. Testes: `pipeline-health.test.ts` (+6: zero `style=`, zero emoji, ícones SVG, `data-color`/`data-status`) + `report-styles.test.ts` (+1: classes do dashboard). Suíte 538×7446 ✅ / tsc 0 / lint ✅.
- ~~`I-8.2` schedule/interactive dashboards -> `buildHtmlPage` (F5-T2 a11y).~~ → **✅ DONE (2026-08-05):** `interactive-mode.ts` `_dashboardQualityGate` + `schedule-handler.ts` weekly-quality `styles:''` → `buildHtmlPage`/`buildCss`/`resolveGeneratedAt` (padrão dos 16 irmãos). D5 determinismo: `report-html.ts` + `backlog-health-renderer.ts` migrados para `resolveGeneratedAt(seed)` (cross-day non-deterministic bug corrigido). Commit `26ef64d6`.

**Acceptance:** `rg` zero CSS inline / `_PIPELINE_CSS` em pipeline-health; zero hex/emoji fora de tokens; a11y (skip-link/focus) preservada. → ✅ VERIFICADO (commit `f430f094`, pushado).

**Auditoria:** commit + gate; regenerar harness (pipeline-health.html). → commit `f430f094`; gate ✅; harness regenerado 41KB `doctype=ok`.
...
- ~~`I-9.4` AQS **24+1** (24 tipos + 1).~~ → **✅ DONE (`35f430cf`):** scorecard contabiliza os **24 specs** explicitamente — 19 com AQS real (`report-html`, `pipeline-health` + 17 fixtures) + 5 registrados como `unscored` documentado (2 orquestradores `schedule-handler`/`interactive-mode` = `nao-aplicavel`; 3 pr-report markdown/job-summary/html = `gate-proprio` teto T2). Nenhum score fabricado (Rule 25); novo campo `unscored` aditivo no scorecard (retrocompatível).
- ~~`I-9.5` Remoção de artefato com **AQS < 60**.~~ → **✅ DONE (`78da8725`):** scorecard puro marca `removable` (testado: AQS<60 → fail/removível); runner emite `process.exitCode = 1` quando há removível (CI não mascara artefato degradado, Rule 25). Hoje os 19 scoreados são ≥75 → nenhum removível → exit 0.
...
| 2026-08-05 | II | protocolo | ✅ | `dev/docs/internal/VALIDATION-PLAN.md` — invariante, fixtures-only, fluxo por fase (harness+D1+D2+D3+sha256+re-prova), scorecard, anti-patterns. Commit `128bd1f6` |
| 2026-08-05 | III | validação determinística | ✅ | `dev/docs/internal/VALIDATION-REPORT.md` — 21 hashes sha256 de `reports/validation/*.html` idênticos entre 3 runs (suíte 543 files/7481 tests, EXIT=0); determinismo PROVADO (diff de lists ordenadas vazio); hashes conferem com `sha256sum` real; zero secrets no report. Causa-raiz `execFileSync` herdando `.env.local`/`.env` resolvida na origem via `_env-pristine.ts` (env snapshot pré-`ensureDotenv`). Commit `128bd1f6` |
| 2026-08-05 | IV | encerramento | ✅ | sanar claims stale (§2/§3/§4/§5/§9/§11.1) + INTEGRATED-PLAN 30+ refs + CONTENT-SPEC gap table + completion-plan 28→24 + fechar Status + auditoria final |
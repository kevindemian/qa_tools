...
| F0 | F0-T8 | 4 consumers passam `computed` real (case17, gen-report, harness, failure-analysis); remover mock de `generateHtmlReport` em case17.test (13× mock-theater) | `test-report.html` sem ERROR-PAGE |
...
| — | F0-T9 (B10/N3 — remover `generateReportWithFallback` + `generateCoverageHtml`; migrar failure-analysis) | ⏳ PENDENTE | inline `generateHtmlReport` estrito; deletar dead code; barrel `report-generator.ts`; madge/tsc sem refs |
| — | F0-T8 (B6/P2 — 4 consumers passam `computed`; remover mock de `generateHtmlReport` em case17.test) | ⏳ PENDENTE | `case17.ts:369`, `gen-report-complete.ts:165`, `harness:550`, `failure-analysis.ts:198`; `test-report.html` real sem ERROR-PAGE |

...
1. **Estado atual:** Fase A concluída (primitives) · Plan v2 registrado (§7) · F1-T1 (B1) ✅ · F1-T2 (B2) ✅ · F1-T3 (B3 merge release-score) ✅ · **Próxima: F1-T4** — ver PROGRESS abaixo.
...
5. **Arquivos-chave:**
   - SSOT compute: `shared/data-hub/hub.ts` (`computeMetrics:794-914`, factories `:446-510`), `shared/data-hub/factory.ts:110` (`createDataHubFromParseResult`), `shared/types/data-hub.ts:694-769` (contrato `ComputedMetrics`).
...
| — | F1-T4 a F7 | ⏳ PENDENTE | seguir §8/§7 |

> **Nota worktree:** `git_triggers/__tests__/pipeline-health.test.ts` e `jira_management/__tests__/jira_link_manager.test.ts` têm alterações não commitadas PRÉ-EXISTENTES (parametrização de testes) — não reverter; manter intactas.

...
| F0 | F0-T8 | **SSOT-ificação failure-analysis (N7):** contrato `analyzeFailuresWithReport(dataHub, context?)` — `tests` derivado de `dataHub.computed.metricsRuns[0].tests` (fonte única); `offerPipelineFailureAnalysis(parsed, dataHub?, cb?)` propaga hub nos 3 callers (pipeline-handler/batch-mode/smoke-pipeline; defensivo `DataHubImpl.createFromParseResult`); remover 2 mocks de case17.test (`analyzeFailuresWithReport` + `generateHtmlReport`); + `case17.ts:369`, `gen-report-complete.ts:165`, `harness:550` | zero error page com dado disponível; `test-report.html` real sem ERROR-PAGE; tests sem mock-theater |
...
| — | F0-T9 (N8 — dead code: `generateReportWithFallback` + `generateCoverageHtml` + barrel → DELETAR; inline `generateHtmlReport`; failure-analysis migrado; testes em 4 arquivos; docs) | ⏳ PENDENTE | inline estrito; zero refs madge/tsc; suíte verde; docs ghost corrigidas |
| — | F0-T8 (N7/B6/P2 — SSOT-ificação failure-analysis: contrato `analyzeFailuresWithReport(dataHub, context?)`; hub nos 3 callers llm-pipeline; 2 mocks case17.test removidos; case17/gen-report/harness) | ⏳ PENDENTE | zero error page com dado disponível; `test-report.html` real sem ERROR-PAGE |

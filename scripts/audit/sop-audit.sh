...
run_phase_01() {
    ...
    cmd "0.1.3 — Casts/suppressions" \
        "grep -rnP ${EXCLUDE_PATHS} 'as any|as unknown|@ts-ignore|@ts-expect-error|eslint-disable' $src 2>/dev/null || true"
    cmd "0.1.4 — Object.entries propaga any" \
        "grep -rnP ${EXCLUDE_PATHS} 'Object\.entries\(' $src 2>/dev/null || true"
    cmd "0.1.5 — I/O sem try/catch" \
        "grep -rnP ${EXCLUDE_PATHS} 'readFile|writeFile|mkdir|unlink|existsSync' $src 2>/dev/null || true"
    cmd "0.1.6 — Catch vazio" \
        "grep -rnP ${EXCLUDE_PATHS} 'catch\s*\{' $src 2>/dev/null || true"
    cmd "0.1.10 — DepWall: imports diretos de libs externas" \
        "grep -rnP ${EXCLUDE_PATHS} \"^import .* from '\" $src 2>/dev/null | grep -vP '\.js|\.json' | grep -vP 'shared/deps' || true"
    cmd "0.1.12 — Constantes mágicas" \
        "grep -rnP ${EXCLUDE_PATHS} '[^a-zA-Z]\d{4,}[^a-zA-Z]' $src 2>/dev/null || true"
    cmd "0.1.13 — Divisão sem guarda zero/NaN" \
        "grep -rnP ${EXCLUDE_PATHS} '/ [a-zA-Z_]\w+' $src 2>/dev/null || true"
    cmd "0.1.14 — Object.values/keys/entries sem ?? {}" \
        "grep -rnP ${EXCLUDE_PATHS} 'Object\.(values|keys|entries)\(' $src 2>/dev/null || true"
    cmd "0.1.15 — || 0 / || '' / || -1 (falsy fragility)" \
        "grep -rnP ${EXCLUDE_PATHS} '\|\| 0\b|\|\| \"\"|\|\| -1' $src 2>/dev/null || true"
    cmd "0.1.17 — reduce sem initial value" \
        "grep -rnP ${EXCLUDE_PATHS} '\.reduce\([^,)]*\)' $src 2>/dev/null || true"
    cmd "0.1.18 — JSON.parse(JSON.stringify(x))" \
        "grep -rnP ${EXCLUDE_PATHS} 'JSON\.parse\(JSON\.stringify\(' $src 2>/dev/null || true"
    cmd "0.1.19 — typeof x === 'object' sem excluir null" \
        "grep -rnP ${EXCLUDE_PATHS} \"typeof .* === 'object'\" $src 2>/dev/null || true"
    cmd "0.1.20 — for...in em array" \
        "grep -rnP ${EXCLUDE_PATHS} 'for\s*\(.*\s+in\s+' $src 2>/dev/null || true"
    cmd "0.1.21 — new Date(string) sem validação" \
        "grep -rnP ${EXCLUDE_PATHS} 'new Date\(' $src 2>/dev/null || true"
    cmd "0.1.22 — console.log/warn/error fora de logger" \
        "grep -rnP ${EXCLUDE_PATHS} 'console\.(log|warn|error)' $src 2>/dev/null | grep -v 'logger.ts' | grep -v 'output.ts' || true"
    cmd "0.1.23 — Parâmetro reatribuído" \
        "grep -rnP ${EXCLUDE_PATHS} '^\s+\w+\s*=\s*[a-z_A-Z]' $src 2>/dev/null | grep -vP '^\s+(const|let|var)' || true"
    cmd "0.1.24 — var vs let/const" \
        "grep -rnP ${EXCLUDE_PATHS} '^var ' $src 2>/dev/null || true"
    cmd "0.1.25 — TODO/FIXME/HACK/XXX/WORKAROUND" \
        "grep -rnP ${EXCLUDE_PATHS} 'TODO|FIXME|HACK|XXX|WORKAROUND' $src 2>/dev/null || true"
...
run_phase_2() {
    ...
    cmd "T17 — Env var dependency" \
        "grep -rnP ${EXCLUDE_PATHS} 'process\.env\[' $src 2>/dev/null || true"
    cmd_presence "T18 — Error handling (throws)" \
        "grep -rnP ${EXCLUDE_PATHS} 'throw\s+(new\s+)?[A-Z]\w*(Error)?' $src 2>/dev/null || true"
...
run_phase_3() {
    ...
    cmd_info "D4.1 — Loops/iterators" \
        "grep -rnP ${EXCLUDE_PATHS} 'for|while|map|filter|reduce|forEach' $src 2>/dev/null | head -20 || true"
    cmd_count "D4.6 — Complexidade (branching keywords)" \
        "grep -rhP 'if|else|for|while|switch|catch' $src 2>/dev/null | wc -l || echo 0" 20

    section_header "Phase 3 — D8: Domain Adequacy"
    cmd "D8.3 — NaN propagation (Math aninhado)" \
        "grep -rnP ${EXCLUDE_PATHS} 'Math\.(floor|round|ceil|trunc)\(.*Math\.' $src 2>/dev/null || true"
    cmd "D8.4 — Falsy coalescence (|| 0 / || \"\")" \
        "grep -rnP ${EXCLUDE_PATHS} '\|\| 0\b|\|\| \"\"\b|\|\| -1\b' $src 2>/dev/null || true"
    ...
    cmd "D9.8 — Math sem isFinite" \
        "grep -rnP ${EXCLUDE_PATHS} 'Math\.(floor|round|ceil|trunc)\(' $src 2>/dev/null || true"

    section_header "Phase 3 — D10: Error & Async Integrity"
    cmd "D10.1 — Catch sem discriminação" \
        "grep -rnP ${EXCLUDE_PATHS} '} catch \(' $src 2>/dev/null || true"
    cmd "D10.2 — return await" \
        "grep -rnP ${EXCLUDE_PATHS} 'return await ' $src 2>/dev/null || true"
    cmd "D10.3 — Promise.all" \
        "grep -rnP ${EXCLUDE_PATHS} 'Promise\.all\(' $src 2>/dev/null || true"
    cmd "D10.4 — Fire-and-forget (chamada sem await)" \
        "grep -rnP ${EXCLUDE_PATHS} '^\s+\w+\(.*\)[^)]*\s*$' $src 2>/dev/null | grep -vP 'then|catch|await|return\b|typeof|instanceof' || true"
    cmd "D10.5 — instanceof" \
        "grep -rnP ${EXCLUDE_PATHS} 'instanceof' $src 2>/dev/null || true"
    ...
    cmd "D11.4 — console em produção" \
        "grep -rnP ${EXCLUDE_PATHS} 'console\.(log|warn|error)' $src 2>/dev/null | grep -vP 'logger\.ts|output\.ts' || true"
    cmd "D11.5 — Regex / match" \
        "grep -rnP ${EXCLUDE_PATHS} 'new RegExp|\.match\(' $src 2>/dev/null || true"
    cmd "D11.6 — toLowerCase/toUpperCase" \
        "grep -rnP ${EXCLUDE_PATHS} '\.toLowerCase\(\)|\.toUpperCase\(\)' $src 2>/dev/null || true"
    cmd "D11.7 — Imports que deveriam ser type-only" \
        "grep -rnP ${EXCLUDE_PATHS} '^import .* from' $src 2>/dev/null | grep -vP 'import type\b' | head -20 || true"
    ...
    cmd "D12.3 — path.resolve/join" \
        "grep -rnP ${EXCLUDE_PATHS} 'path\.(resolve|join)\(' $src 2>/dev/null || true"
    cmd "D12.4 — import.meta" \
        "grep -rnP ${EXCLUDE_PATHS} 'import\.meta\.(dirname|url)' $src 2>/dev/null || true"
    cmd "D12.5 — os.EOL / hardcoded \\n" \
        "grep -rnP ${EXCLUDE_PATHS} 'os\.EOL|\\\\n' $src 2>/dev/null || true"

    section_header "Phase 3 — D13: Parameter & State Integrity"
    cmd "D13.1 — Parâmetros reatribuídos" \
        "grep -rnP ${EXCLUDE_PATHS} '^\s+\w+\s*=\s*[a-z_A-Z]' $src 2>/dev/null | grep -vP '^\s+(const|let|var|this\.|import|type|interface|function)' || true"
    cmd "D13.2 — Assignment dentro de if" \
        "grep -rnP ${EXCLUDE_PATHS} 'if \(.*\|\|.*=' $src 2>/dev/null || true"
    cmd "D13.3 — var hoisting" \
        "grep -rnP ${EXCLUDE_PATHS} '^var ' $src 2>/dev/null || true"

    section_header "Phase 3 — D11.8: Type-only imports corretos"
    cmd_presence "D11.8 — Type imports corretos" \
        "grep -rnP ${EXCLUDE_PATHS} '^import .* from' $src 2>/dev/null | grep -P '\btype\b' || true"
}

...
## 🩺 Sprint Saúde da Codebase — Lógica de Risco Não-Testada (2026-07-17)

**Origem:** investigação de saúde (não de threshold) motivada pela instrução do usuário: "o objetivo é ter um codebase saudável; threshold é o mínimo". Diagnóstico via `coverage` cruzado com leitura de código: prioridade em **branch coverage** de lógica de segurança/domínio/§25 (silenciamento), não em statements globais.

**Princípio:** nem toda "50% branch" é lacuna de teste — algumas são **ramos defensivos inalcançáveis** (defeito de qualidade), corrigidos na origem (§4), nunca com teste-teatro. Cada caso tratado pela sua natureza real. Cobertura é **consequência**, não meta.

| ID    | Item                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Arquivo(s)                                                           | Status |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------- | ------ |
| SH-1  | 🔧 `failure-analysis`: cobrir §25/degradação de `analyzeFailuresWithReport` (template ausente→fallback; LLM lança→fallback, não silencia) + fallback aninhado de `classifyFailure` (consenso→llmPrompt→UNKNOWN; divergência). Mockar só fronteiras LLM, não a função.                                                                                                                                                                                  | `shared/failure-analysis.ts`, `shared/failure-analysis.test.ts`      | ✅     |
| SH-2a | 🔧 `escape.ts`: DEFEITO — busca linear O(n) `Object.entries().find()` + `?? ch` inalcançável. Corrigido p/ lookup direto `MAP[ch]` (union literal); PBT (fast-check) provando escape total/idempotência; ramo defensivo §24 documentado.                                                                                                                                                                                                               | `shared/escape.ts` (novo test)                                       | ✅     |
| SH-2b | 🔧 `safe-json.ts`: `String(err)` inalcançável via `JSON.parse` nativo (sempre `SyntaxError`). PBT round-trip (wrapper == JSON.parse nativo, inclui -0→0) + fallback; ramo defensivo §24 documentado (defesa em profundidade, não removido).                                                                                                                                                                                                            | `shared/safe-json.ts`, `shared/safe-json.test.ts`                    | ✅     |
| SH-2c | 🔧 `assertions.ts` (path real `shared/test-utils/`): ramo real não-testado é `onNull` omitido. Teste dedicado dos 3 desfechos (presente / null / undefined / falsy-but-present / sem handler).                                                                                                                                                                                                                                                         | `shared/test-utils/assertions.ts` (novo test)                        | ✅     |
| SH-2d | 🔧 `csv-exporter.ts`: `?? ''` (linha 29) inalcançável via `exportMetricsCsv`. Removido código morto — `FlatRow` reificado como interface `{metric, value}` (contrato `ComputedMetrics` não admite ausência); `toCsv` usa acesso direto. Sem `as any`.                                                                                                                                                                                                  | `shared/data-hub/metrics/csv-exporter.ts`                            | ✅     |
| SH-3a | 🔧 `framework-detector`: ramo infra-error (`warn`, 47-48) não testado. Testado 5 kinds (auth/permission/rateLimit/network/server)→`warn`+remediation; notFound/unknown→`debug`. Prova §25: infra-error sempre apresentado.                                                                                                                                                                                                                             | `shared/data-hub/extractors/framework-detector.ts`                   | ✅     |
| SH-3b | 🔧 `env-loader`: zero cobertura em overlay/segredos. fs real (temp): write/apply round-trip, nome inválido→throw, ausente→no-op, malformado→catch loga E re-lança (§25), `warnSecretsInFile`/`hasSecretPattern`, `reloadDotenv`, `loadTestEnv`. `ensureDotenv(projectRoot?)` + `logViaRootLogger` (SRP/DRY).                                                                                                                                           | `shared/env-loader.ts`, `shared/env-loader.test.ts`                  | ✅     |
| SH-4a | 🔧 `llm-review-prompts` (SEM teste, pior branch 25%): novo test cobrindo `getSchemaForType`/`getTypeReviewChecks` p/ 5 ArtifactType + builders com tipos não-analysis + `buildRetryPrompt` com/sem `invalidResponse`.                                                                                                                                                                                                                                  | `shared/llm-review-prompts.ts` (novo test)                           | ✅     |
| SH-4b | 🔧 `llm-review` (208-213,433,449): retry rejeita→`runRetryLoop` catch (`recordLlmFailure`); adversarial não-high→`recordArtifactRejected`; self-review `low`→`recordArtifactRejected`. Teste `llm-review.rejection.test.ts` (LLM é única fronteira mockada).                                                                                                                                                                                           | `shared/llm-review.ts`, `shared/llm-review.rejection.test.ts`        | ✅     |
| SH-5  | 🔧 Extrair guard §25 de `ci-status.ts`/`run-semgrep.ts` `main()` p/ funções puras testáveis (`classifyOutcome`/`buildReport`/`runCiStatus`; `resolveSemgrepBin`/`buildSemgrepArgs`/`classifySemgrepExit`). `main()` vira shell trivial. DECISÃO (opção A, 2026-07-17): `vitest.config.ts` **INALTERADO** — shells permanecem no coverage; a extração de lógica já resolve a lacuna na origem (§4) sem enfraquecer o safety mechanism de coverage (§6). | `scripts/ci-status.ts`, `scripts/run-semgrep.ts`, `vitest.config.ts` | ✅     |

**Legenda:** 📨 pendente · 🔨 em progresso · ✅ concluído

**Validação final (2026-07-17):** `npx tsc --noEmit`=0; `npm run lint`=0 (eslint zero violations + DepWall); suíte completa verde — **7063 pass / 18 skipped / 0 fail** (sem alterar asserts existentes — §19.5). Coverage dos alvos SH medida e real (não teste-teatro): `env-loader.ts` 96% stmts/85% branch, `failure-analysis.ts` 100% stmts/funcs, `safe-json.ts` 100% stmts/funcs (branch 50% = ramo defensivo §24 documentado), `llm-review.ts` cobre os branches de rejeição 407/433/449, `ci-status.ts` 82% stmts/81% branch (funções puras extraídas), `run-semgrep.ts` 56% stmts/73% branch (shell `main()` não excluído — decisão SH-5 opção A). Ramos remanescentes descobertos são defensivos-documentados (§24) ou shells trivialmente finos.

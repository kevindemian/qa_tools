...
| A6  | Docker image + npm package (CI/CD)                                          | P2         | 2h      | 🔷 Feature futura (adiada) | **Não é débito.** Dockerfile + workflow removidos — implementação prematura sem caso de uso concreto. Revisitar quando a versão estiver estável. Empacotar como: (1) imagem Docker para worker serverless, (2) CLI pública via `npm install -g`. |
...
| F6  | report-html.ts extração (chart + sections)                                                 | 🟡 MÉDIO  | ✅     | `shared/report-html.ts`, `shared/report-chart.ts`, `shared/report-sections.ts`, `shared/report-utils.ts`                                                                                                                     |
| F8  | `process.env` → `Config.get()` (shared/, 8 arquivos)                                       | 🟡 MÉDIO  | ✅     | `shared/disk-cache.ts`, `shared/llm-benchmark.ts`, `shared/open.ts`, `shared/output.ts`, `shared/publish.ts`, `shared/report-html.ts`, `shared/temp-dir.ts`                                                                  |
| F9  | `Config` construtor privado                                                                | 🟢 BAIXO  | ✅     | `shared/config.ts`                                                                                                                                                                                                           |
| F10 | ts-prune — filtra falsos positivos "used in module" + 25 exports intencionais (public API) | 🟢 BAIXO  | ✅     | `scripts/check-unused-exports.sh`, `package.json`                                                                                                                                                                            |

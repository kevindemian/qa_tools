...
import { success, error, warn, info, withSpinner } from '../shared/prompt';
...
class TestExecutionCreator {
  ...
  async create(projectName: string, testKeys: string[], csvName: string, titleOverride?: string): Promise<TestExecutionResult> {
    ...
    const fields = await this.jiraResource.getJiraResource<Array<{ id: string; name: string; schema?: { custom?: string } }>>('field');
  ...
  async createWithLinks(projectName: string, testKeys: string[], csvName: string, execOpts?: { title?: string; description?: string }): Promise<TestExecutionResult> {
    ...
    if (result.key && testKeys.length > 0) {
      try {
        ...
        try {
          const te = await this.jiraResource.getJiraResource<{ fields?: { issuelinks?: Array<{ outwardIssue?: { key: string } }> } }>('issue/' + result.key);

import fs from 'fs';
import { info, success, warn } from '../shared/prompt';
...
function _fuzzyMatch(title: string, mappings: MappingItem[]): MappingItem | null {
  ...
  return mappings.find(
    m => m.title.replace(/[^a-zA-Z0-9]/g, '').toLowerCase() === normalized
  ) || null;
}
...
async function createTestExecutionFromResults(
  ...
  const { createTestExecution } = require('./create_tests') as { createTestExecution: Function };
  const te = await createTestExecution(jiraResource, project_name, testKeys, csvName, summary) as { key: string };

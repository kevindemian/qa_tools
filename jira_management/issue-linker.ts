import { success, info, isQuiet, onError } from '../shared/prompt';
...
class IssueLinker {
  ...
  async associatePrecondition(test: TestCase, issueKey: string, opLog: { info: (msg: string, meta?: Record<string, unknown>) => void }): Promise<ActionResult | null> {
  ...
  async updateCrossReferences(tests: TestCase[], ids: string[]): Promise<void> {
    ...
    for (const group of Object.values(groups)) {
      ...
      for (const member of group.members) {
        ...
        try {
          const current = await this.jiraResource.getJiraResource<{ fields?: { description?: string } }>('issue/' + member.id);
          ...
          crossLog.error('Falha ao atualizar descrição de ' + member.id + ' no grupo "' + group.name + '"', { status });

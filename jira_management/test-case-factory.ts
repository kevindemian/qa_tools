...
class TestCaseFactory {
  ...
  async createIssue(testData: Record<string, unknown>, testTitle: string, testIdx: number, totalTests: number, opLog: { info: (msg: string, meta?: Record<string, unknown>) => void }): Promise<CreateIssueResult> {
    try {
  ...
  async postSteps(issueKey: string, test: TestCase, opLog: { info: (msg: string, meta?: Record<string, unknown>) => void }): Promise<StepsResult | null> {
    let abortSteps = false;
    const stepBar = !isQuiet() ? new ProgressBar(test.steps.length, { width: 15 }) : null;
    for (let i = 0; i < test.steps.length; i++) {
      try {
        await this.jiraResourceXray.postJiraResource('test/' + issueKey + '/steps', { index: i + 1, ...test.steps[i] });

...
export function buildDeveloperProfile(
    ...
    try {
        ...
        rootLogger.error('Failed to build developer profile: ' + (err instanceof Error ? err.message : String(err)));
        return {
            authors: [],
            totalAuthors: 0,
            totalFailures: 0,
...
export function generateDeveloperProfileHtml(
    ...
    try {
        ...
        rootLogger.error('Failed to generate developer profile HTML: ' + msg);
        return buildErrorPage('Error generating developer profile', 'Error generating developer profile');
    }
}

...
describe('ParseTests', () => {
    it('returns empty array for null', () => {
        expect(parseTests(null)).toStrictEqual([]);
    });

    it('returns empty array for non-object', () => {
        expect(parseTests('string')).toStrictEqual([]);
    });

    it('returns the array directly when artifact is an array', () => {
        const arr = [{ title: 'Test' }];

        expect(parseTests(arr)).toStrictEqual(arr);
    });
    ...
    it('returns empty array when object has no tests key', () => {
        expect(parseTests({ foo: 'bar' })).toStrictEqual([]);
    });
});

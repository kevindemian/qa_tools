...
describe('formatDateISO', () => {
    it('formats a known date as YYYY-MM-DD', async () => {
        const d = new Date(2026, 4, 30); // May 30 2026
        expect(formatDateISO(d)).toBe('2026-05-30');
    });

    it('pads single-digit month and day with zero', async () => {
    ...
    it('formats last day of year', async () => {
        const d = new Date(2026, 11, 31);
        expect(formatDateISO(d)).toBe('2026-12-31');
    });

    it('uses today when no date argument given', async () => {
        const today = new Date();
        const expected = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`;
        expect(formatDateISO()).toBe(expected);
    });

    it('produces same output as toISOString slice(0,10) for local dates', async () => {
        const d = new Date(2026, 4, 30, 12, 0, 0);
        expect(formatDateISO(d)).toBe('2026-05-30');
    });

    it('handles first day of year', async () => {

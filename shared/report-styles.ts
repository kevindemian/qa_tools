...
export function buildCss(): string {
    ...
    return (
        BASE_LAYOUT_CSS +
        `.card .value.pass { color: ${t.colors.success}; }
.card .value.fail { color: ${t.colors.error}; }
.card .value.skip { color: ${t.colors.warn}; }
.failed-summary { box-shadow: inset 4px 0 0 0 ${t.colors.error}; }
` +
        TABLE_CSS +
        ERROR_HISTORY_CSS +
        TABS_SIDEBAR_CSS +
        DETAIL_CSS +
        `.rate-good { color: ${t.colors.success}; }
.rate-warn { color: ${t.colors.warn}; }
.rate-bad { color: ${t.colors.error}; }
` +
        PRINT_MEDIA_CSS +
        DARK_CSS
    );

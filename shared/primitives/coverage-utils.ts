/** Pure computation utilities for coverage gap analysis.
 * DataHub SSOT — all coverage gap computation flows through this module.
...
export function checkQualityGate(
    ...
    const safeMin = Number.isFinite(minCoveragePct) ? minCoveragePct : COVERAGE_GATE_DEFAULT;
    const failingEpics: string[] = [];
    for (const [key, epic] of Object.entries(byEpic)) {
        if (key === '__no_epic__') continue;
        if (epic.rawPct < safeMin) {
            failingEpics.push(key);
            epic.gatePass = false;
        }
    }
    return { minCoveragePct: safeMin, failingEpics };
}

...
export function generateFlakinessHtml(flaky: FlakinessEntry[], title?: string): string {
    try {
        ...
        html += '<title>' + escapeHtml(pageTitle) + '</title><style>';
        html += `
...
`;
        ...
        html +=
            ...
            '</div></div>';
        html +=
            '<div class="card"><div class="label">Threshold</div><div class="value">>' +
            THRESHOLD_PCT +
            '%</div></div>';
        html +=
            '<div class="card"><div class="label">All Candidates</div><div class="value">' +
            flaky.length +
            '</div></div>';
        html += '</div>';

        if (high.length === 0) {
            html += '<p>No tests exceed the ' + THRESHOLD_PCT + '% flakiness threshold.</p>';
        } else {
            html +=

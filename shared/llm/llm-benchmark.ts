...
import { gracefulExit } from '../ui/cli_base.js';
...
import { computeCoverageMetrics, type BenchmarkMetrics } from '../quality/benchmark-metrics.js';
import { checkQualitySignals } from '../quality/quality-suggester.js';
import type { QualitySignal } from '../quality/quality-suggester.js';
...
function printResults(results: BenchmarkResult[]): void {
    ...
    defaultOutput.print(
        'Total: ' +
            ...
            failed +
            ' | Rate: ' +
            calcRunPassRate({ passed, failed: total - passed }) +
            '%',
    );
    defaultOutput.print('');
...
export async function runBenchmark(): Promise<void> {
    ...
    if (total > 0) {
        const passRate = calcRunPassRate({ passed: passCount, failed: total - passCount });
        if (passRate < 50) {
            benchmarkSignals.push({
                severity: 'critical',
                source: 'benchmark',
                message: `Benchmark pass rate ${passRate.toFixed(0)}% (${passCount}/${total}) abaixo do esperado.`,
            ...
            benchmarkSignals.push({
                severity: 'warning',
                source: 'benchmark',
                message: `Benchmark pass rate ${passRate.toFixed(0)}% (${passCount}/${total}) abaixo do ideal.`,

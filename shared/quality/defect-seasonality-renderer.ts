...
function getVsAvgLabel(total: number, avgTotal: number): string {
    if (total > avgTotal * 1.2) return `${icon('alert-circle', 14)} Above`;
    if (total < avgTotal * 0.8) return `${icon('trending-down', 14)} Below`;

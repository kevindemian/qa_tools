...
import { ask, askMultiline, askConfirm, info, printError, title, warn } from '../ui/prompt.js';
...
export function compose(report: BugReport, generatedAt?: string): string {
    const lines = [
    ...
    lines.push('', '---', `Generated by QA Tools at ${resolveGeneratedAt(generatedAt)}`);
    return lines.join('\n');
}

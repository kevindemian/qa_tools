...
const VALIDATION_TIMEOUT_MS = parseInt(
  process.env.VALIDATION_TIMEOUT_MS || "30000",
  10,
);
const STDIN_TIMEOUT_MS = parseInt(process.env.STDIN_TIMEOUT_MS || "30000", 10);
const MAX_DIFF_LINES = parseInt(process.env.MAX_DIFF_LINES || "10000", 10);
const MAX_DIFF_BUFFER = 100 * 1024 * 1024; // 100MB
const COMMAND_TIMEOUT_MS = parseInt(
  process.env.COMMAND_TIMEOUT_MS || "10000",
  10,
);
...
const FORBIDDEN_PATTERNS: RegExp[] = [
  ...
  /(?<!(?:avoid|never use|no|without a|eliminate|remove|prevent)\s{0,20})\bworkaround\b/i,
  /(?<!(?:avoid|never|without)\s{0,10})\bbypass\b/i,
  /\bcircumvent\b/i,
  /\bevade\b/i,
  /\bkludge\b/i,
  /\bmonkey\s*patch\b/i,
  ...
  /(?<!(?:avoid|never use|don't use|do not use|remove|delete)\s{0,20})@ts-ignore\b/i,
  /(?<!(?:avoid|never use|don't use|do not use|remove|delete)\s{0,20})@ts-expect-error\b/i,
  /@ts-nocheck\b/,
...
function validateTsJsBlock(code: string): string[] {
  return DANGEROUS_TS_CODE_PATTERNS.filter((p) => p.test(code)).map(
    (p) => p.source,
  );
}
...
function validateCodeBlocks(response: string): string[] {
  ...
  for (const block of blocks) {
    if (["typescript", "javascript", "ts", "js"].includes(block.language)) {
      all.push(
        ...validateTsJsBlock(block.content).map(
          (v) => `[${block.language}] ${v}`,
        ),
      );
    }
...
function detectFileWrites(text: string): FileWriteDetection[] {
  ...
  const patterns = [
    ...
    {
      regex:
        /python3?\s+-c\s+['"`](?:open\(['"`])([^'"`]+)['"`](?:,\s*['"]w['"]\)\.write\(['"`])([^'"`]+)['"`]\)/gi,
      extractor: (m: RegExpExecArray) => ({
        command: m[0],
        targetFile: m[1],
        content: m[2],
    ...
    {
      regex:
        /node\s+-e\s+['"`]fs\.writeFileSync\(['"`]([^'"`]+)['"`],\s*['"`]([^'"`]+)['"`]\)/gi,
      extractor: (m: RegExpExecArray) => ({
        command: m[0],
        targetFile: m[1],
        content: m[2],
...
function validateIndirectFileWrites(response: string): string[] {
  ...
  for (const write of writes) {
    if (write.content)
      all.push(
        ...validateTsJsBlock(write.content).map(
          (v) => `file write to ${write.targetFile}: ${v}`,
        ),
      );
  }
  return all;
...
function detectArchitecturalWorkarounds(response: string): string[] {
  return ARCHITECTURAL_WORKAROUND_PATTERNS.filter((p) => p.test(response)).map(
    (p) => p.source,
  );
}
...
function validateMultiCommand(command: string): string[] {
  ...
  return recursionDepthStorage.run(0, () => {
    ...
    for (const subCmd of subCommands) {
      ...
      if (trimmed) {
        if (depth >= MAX_RECURSION_DEPTH) {
          violations.push(
            `Max recursion depth exceeded for command: ${trimmed.substring(0, 100)}`,
          );
          continue;
        }
        const subViolations = recursionDepthStorage.run(depth + 1, () =>
          validateCommandContent(trimmed),
        );
...
const commandCache = new Map<
  string,
  { violations: string[]; timestamp: number }
>();
...
function saveRejectedHash(hash: string, patterns: string[]): void {
  try {
    ...
    writeFileSync(
      CACHE_FILE,
      JSON.stringify({ entries, lastUpdated: now }, null, 2),
      "utf-8",
    );
  } catch {
    /* sem permissão */
...
export function validateResponse(response: string): boolean {
  ...
  if (cached)
    throw new Error(
      `RESPOSTA REJEITADA (cache): ${cached.patterns.join("; ")}`,
    );
  ...
  for (const pattern of FORBIDDEN_PATTERNS)
    if (pattern.test(response))
      violations.push({ pattern: pattern.source, severity: "block" });
  ...
  if (intent.found)
    violations.push({ pattern: intent.message, severity: "block" });
  ...
  for (const v of validateCodeBlocks(response))
    violations.push({ pattern: v, severity: "block" });
  for (const v of validateIndirectFileWrites(response))
    violations.push({ pattern: v, severity: "block" });
...
export function getViolationStats(): {
  ...
  try {
    if (existsSync(VIOLATION_LOG_FILE)) {
      const lines = readFileSync(VIOLATION_LOG_FILE, "utf-8")
        .trim()
        .split("\n")
        .filter(Boolean);
...
function parseGitDiff(diff: string): DiffLine[] {
  ...
  for (let lineNumber = 0; lineNumber < rawLines.length; lineNumber++) {
    ...
    if (rawLine.startsWith("@@")) {
      ...
      if (match) currentOriginalLine = parseInt(match[1]!, 10) - 1;
      lines.push({
        lineNumber,
        content: rawLine,
        type: "context",
...
function runCheck(diff: string, quiet: boolean = false): CheckResult {
  ...
  if (addedLines.length === 0 && issues.length === 0) {
    if (!quiet) console.log("Nenhuma adição para validar.");
    return {
      valid: true,
      issues: [],
  ...
  if (addedLines.length > MAX_DIFF_LINES) {
    if (!quiet)
      console.warn(
        `⚠️ Diff muito grande (${addedLines.length} linhas). Limitando a ${MAX_DIFF_LINES}.`,
      );
...
async function runCheckCommitMsg(quiet: boolean = false): Promise<CheckResult> {
  const gitDir = join(process.cwd(), ".git", "COMMIT_EDITMSG");
  ...
  try {
    ...
    const msg = readFileSync(gitDir, "utf-8");
    const cleanMsg = msg
      .split("\n")
...
async function runHook(): Promise<void> {
  ...
  if (args.includes("--check")) {
    ...
    if (args.includes("--staged")) {
      try {
        ...
        if (!quiet && !outputJson)
          console.error("ERRO: não está em um repositório git");
        if (outputJson)
          console.log(
            JSON.stringify({
              valid: false,
    ...
    const stats = {
      linesValidated:
        diffResult.stats.linesValidated + commitResult.stats.linesValidated,
      timeMs: Date.now() - startTime,
    };
  ...
  if (args.includes("--file")) {
    ...
    if (idx >= args.length) {
      if (!quiet && !outputJson)
        console.error("ERRO: --file requer um caminho");
      process.exit(1);
    }
    try {
      ...
      if (!quiet && !outputJson)
        console.error(`ERRO: não foi possível ler arquivo: ${args[idx]}`);
      process.exit(1);
    }
    ...
    if (idx >= args.length) {
      if (!quiet && !outputJson)
        console.error("ERRO: --response requer uma string");
      process.exit(1);
    }
    response = args[idx] ?? "";
  } else {
  ...
  if (!response || response.trim().length === 0) {
    ...
    if (outputJson)
      console.log(JSON.stringify({ valid: false, error: "Empty response" }));
    process.exit(1);
  }
  ...
  try {
    ...
    if (outputJson)
      console.log(JSON.stringify({ valid: false, error: msg, requiresReview }));

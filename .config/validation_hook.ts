...
const WORKAROUND_PATTERNS: RegExp[] = [
    ...
    // PT-BR
    /\b(?:usar?|aplicar?|tentar?|adicionar?)\s+(?:uma?\s+)?gambiarra\b/i,
...
const INDIRECT_BYPASS_PATTERNS: RegExp[] = [
    ...
    /\bassume\s+(?:it'?s\s+)?(?:correct|valid|safe|works)\b/i,
...
export function validateResponse(response: string): boolean {
    ...
    const block = (p: string) => violations.push({ pattern: p, severity: 'block' });
    const review = (p: string) => violations.push({ pattern: p, severity: 'review' });

    // Padrões com verificação de contexto (isNegated + isInCorrectiveContext)
    for (const v of checkWithContext(response, WORKAROUND_PATTERNS)) block(v);
    for (const v of checkWithContext(response, BYPASS_PATTERNS)) block(v);
    for (const v of checkWithContext(response, SUPPRESSION_PATTERNS)) block(v);
    for (const v of checkWithContext(response, TS_SUPPRESSOR_PATTERNS)) block(v);

    // Alta especificidade — sem necessidade de contexto
    for (const p of TEMPORARY_PATTERNS) if (p.test(response)) block(p.source);
    for (const p of DEFERRAL_PATTERNS) if (p.test(response)) block(p.source);
    for (const p of TECH_DEBT_ACCEPTANCE_PATTERNS) if (p.test(response)) block(p.source);
    for (const p of SPECULATION_PATTERNS) if (p.test(response)) block(p.source);
    for (const p of SECURITY_DISABLE_PATTERNS) if (p.test(response)) block(p.source);
    for (const p of TSCONFIG_DISABLE_PATTERNS) if (p.test(response)) block(p.source);
    for (const p of TEST_SKIP_PATTERNS) if (p.test(response)) block(p.source);
    for (const p of DANGEROUS_PHRASES) if (p.test(response)) block(p.source);
    for (const p of INDIRECT_BYPASS_PATTERNS) if (p.test(response)) block(p.source);
    for (const p of RESPONSIBILITY_AVOIDANCE_PATTERNS) if (p.test(response)) block(p.source);

    // Heurística de intenção
    const intent = hasDangerousIntent(response);
    if (intent.found) block(intent.message);

    // Mock factories
    for (const v of findMockFactoryViolations(response)) review(`mock_factory: ${v.pattern}`);

    // Config externa
    for (const p of ARCHITECTURAL_PATTERNS) if (p.test(response)) block(p.source);

    // Blocos de código (zero FP — são tokens de código)
    for (const v of validateCodeBlocks(response)) block(v);

    // Densidade em blocos de código
    const density = hasDangerousCodeDensity(response);
    if (density.found) block(`dangerous_code_density: ${density.score}%`);

    // Escrita indireta de arquivos
    for (const v of validateIndirectFileWrites(response)) block(v);
...
function runCheck(diff: string, quiet = false): CheckResult {
    ...
    const global = sanitizeAndReject(diff);
    if (!global.valid)
        issues.push({
            line: 1,
            error: global.error ?? 'Erro estrutural',
            severity: global.requiresHumanReview ? 'review' : 'block',
        });
    const parsed = parseGitDiff(diff);
    const added = parsed.filter((l) => l.type === 'added');
    const hasDiff = parsed.some((l) => l.type === 'added' || l.type === 'removed');
    if (!hasDiff && !issues.length) {
        if (!quiet) console.log('Nenhuma alteração para validar.');
        return {
            valid: true,
            issues: [],
            requiresReview: false,
            stats: { linesValidated: 0, timeMs: Date.now() - t0 },
        };
    }
    if (added.length > MAX_DIFF_LINES) {
        if (!quiet) console.warn(`⚠️ Diff grande (${added.length} linhas). Limitando a ${MAX_DIFF_LINES}.`);
        added.length = MAX_DIFF_LINES;
    }
    const seen = new Set<string>();
    const lineIdx = buildOriginalLineIndex(parsed);
    for (const line of added) {
        const li = lineIdx.get(line.originalLineNumber);
        if (li === undefined) continue;
        const ctx = getContextWindow(parsed, li, 3);
        const h = getResponseHash(ctx);
        if (seen.has(h)) continue;
        seen.add(h);
        const r = sanitizeAndReject(ctx);
        if (!r.valid)
            issues.push({
                line: line.originalLineNumber,
                error: r.error ?? 'Unknown',
                severity: r.requiresHumanReview ? 'review' : 'block',
            });
    }
    return {
        valid: !issues.length,
        ...
        stats: { linesValidated: seen.size, timeMs: Date.now() - t0 },
    };
}
...
async function runCheckCommitMsg(quiet = false): Promise<CheckResult> {
    ...
    try {
        ...
        const r = await Promise.race([
            Promise.resolve(sanitizeAndReject(clean)),
            new Promise<ValidationResult>((_, rej) =>
                setTimeout(() => rej(new Error('Timeout')), VALIDATION_TIMEOUT_MS),
            ),
        ]);
        if (!r.valid)
            return {
                valid: false,
                issues: [
                    { line: 0, error: r.error ?? 'Unknown', severity: r.requiresHumanReview ? 'review' : 'block' },
                ],
                requiresReview: r.requiresHumanReview ?? false,
        ...
        if (msg === 'Timeout')
            return {
                valid: false,
                issues: [{ line: 0, error: 'Timeout validating commit message', severity: 'block' }],
                requiresReview: false,

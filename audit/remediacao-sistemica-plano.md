...
| **7** | C11/#C2 Weekly/Interactive: injetar dados reais do DataHub (backlog via `jiraIssues` → `mapJiraIssuesToBacklogHealth`); empty-state explícito. AI via `aiRecords` → `convertGenerationRecordsToFeedback`. Requisitos via `calculateRequirementScores`. | Seções Backlog Health/AI/Requirements com dados reais; `noData: true` quando sem issues/records. | `fix(weekly): fontes reais DataHub (backlog/AI/req), empty-state explícito` |
...
### Fase 7 (#C11/#C2 Weekly/Interactive — fontes reais DataHub) — CONCLUÍDA (2026-07-21)

- **Backlog Health (#C11):** `schedule-handler.ts`/`interactive-mode.ts` agora consomem `hub.raw.jiraIssues` via `mapJiraIssuesToBacklogHealth()` adapter; `analyzeBacklogHealth` recebe issues reais → `noData: true` quando array vazio (sem fabricação de 100%). Test `schedule-handler` mock inclui `mapJiraIssuesToBacklogHealth`.
- **AI Effectiveness:** `aiRecords` do DataHub convertido via `convertGenerationRecordsToFeedback` (novo em `ai-effectiveness.ts`) → `computeAiEffectiveness`; `requirement-score.ts` recebe `aiRecords` direto via `calculateRequirementScores`.
